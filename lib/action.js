...
class Action {
  ...
  httpGet (url, options) {
    ...
    return axios.get(url, opts)
      .then(r => {
        ...
        this.getTime += hrtime[0] * 1e3 + hrtime[1] / 1e6;
        this._common(r);
        return r;
      })
      .finally(() => {
  ...
  httpPost (url, req, options) {
    ...
    return axios.post(url, req, options).then(r => {
      hrtime = process.hrtime(hrtime);
      this.postTime += hrtime[0] * 1e3 + hrtime[1] / 1e6;
      this._common(r);
      return r;
    }).finally(() => {
      this.inFlight -= 1;
  ...
  httpDelete (url, options) {
    ...
    return axios.delete(url, options).then(r => {
      hrtime = process.hrtime(hrtime);
      this.deleteTime += hrtime[0] * 1e3 + hrtime[1] / 1e6;
      this._common(r);
      return r;
    }).finally(() => {
      this.inFlight -= 1;
  ...
  wasSampled (headers) {
    ...
    if (this.agent === 'appoptics') {
      if (!headers['x-trace'] || !headers['x-trace'].startsWith('2B')) {
    ...
    if (headers && headers['x-trace']) {
      return headers['x-trace'].endsWith('01');
    }
    return false;
  }
